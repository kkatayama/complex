...
class CustomAuthManager {
  ...
  Future signOut() async {
    ...
    comPlexAuthUserSubject.add(
      ComPlexAuthUser(loggedIn: false),
    );
    persistAuthData();
  }

  Future<ComPlexAuthUser?> signIn({
    String? authenticationToken,
    String? refreshToken,
    DateTime? tokenExpiration,
    String? authUid,
  ...
  ComPlexAuthUser? _updateCurrentUser({
    String? authenticationToken,
    String? refreshToken,
    DateTime? tokenExpiration,
    String? authUid,
    ...
    final updatedUser = ComPlexAuthUser(
      loggedIn: true,
      uid: authUid,
      userData: userData,
    );
    comPlexAuthUserSubject.add(updatedUser);
    persistAuthData();
    return updatedUser;
  }
  ...
  Future initialize() async {
    ...
    try {
      ...
      userData = _prefs.getString(_kUserDataKey) != null
          ? AuthDataStruct.fromSerializableMap(
